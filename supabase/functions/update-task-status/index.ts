...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (allParkedTasks.length > 0) {
      ...
      const { data: existingNotifs } = await supabase.from('notifications').select('related_entity_id').in('related_entity_id', taskIds).like('message', '%Parked for more than 3 days%')
    ...
    const criticalWoRaw = await supabase.from('wo_tasks').select('id, assigned_to, task_name, department').lt('finish_date', fiveDaysAgoStr).neq('status', 'complete')
    const criticalWo = (criticalWoRaw.data || []).map(t => ({ ...t, type: 'wo_tasks' }))

    const criticalProdRaw = await supabase.from('production_tasks').select('id, assigned_to, task_name, department, sub_department').lt('finish_date', fiveDaysAgoStr).neq('status', 'complete')
    const criticalProd = (criticalProdRaw.data || []).map(t => ({ ...t, type: 'production_tasks' }))

    const criticalPurchRaw = await supabase.from('purchasing_tasks').select('id, assigned_to, component_name').lt('finish_date', fiveDaysAgoStr).neq('status', 'complete')
    const criticalPurch = (criticalPurchRaw.data || []).map(t => ({ ...t, type: 'purchasing_tasks', task_name: t.component_name, department: 'Purchasing', sub_department: null }))
    ...
    if (allCriticalTasks.length > 0) {
      ...
      const notifiedSet = new Set(existingCriticalNotifs?.map(n => `${n.related_entity_id}-${n.user_id}`) || [])
      ...
      if (criticalNotificationsToInsert.length > 0) {
        const { error: notifyCriticalError } = await supabase.from('notifications').insert(criticalNotificationsToInsert)
