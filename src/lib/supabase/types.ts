...
export type Database = {
  ...
  public: {
    Tables: {
      ...
      purchasing_task_comments_history: {
        Row: {
          author: string | null
          comment: string
          created_at: string | null
...
// Table: customers
//   id: uuid (not null, default: gen_random_uuid())
//   customer_id: text (not null, default: ('CUST-'::text || (nextval('customers_id_seq'::regclass))::text))
...
//   monthly_contracted_hours: numeric (nullable, default: 160)
//   hours_per_day: numeric (nullable, default: 8)
//   updated_at: timestamp with time zone (nullable, default: now())
// Table: late_card_pulls
//   id: uuid (not null, default: gen_random_uuid())
//   date: date (nullable)
//   part_number: text (nullable)
//   area_supervisor: text (nullable)
//   occurrence_description: text (nullable)
//   status: text (nullable, default: 'pending'::text)
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
...
//   status: expedite_status_enum (nullable, default: 'pending'::expedite_status_enum)
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
...
//   changed_by: uuid (nullable)
//   changed_at: timestamp with time zone (nullable, default: now())
// Table: purchasing_task_comments_history
//   id: uuid (not null, default: gen_random_uuid())
//   task_id: uuid (not null)
//   comment: text (not null)
//   author: uuid (nullable)
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
//   deleted_at: timestamp with time zone (nullable)
// Table: purchasing_tasks
//   id: uuid (not null, default: gen_random_uuid())
//   wo_id: uuid (not null)
//   component_name: text (not null)
...
//   start_date: date (nullable)
//   finish_date: date (nullable)
//   status: purchasing_task_status_enum (nullable, default: 'not_started'::purchasing_task_status_enum)
//   assigned_to: uuid (nullable)
//   comments: text (nullable)
//   is_completed: boolean (nullable, default: false)
//   completion_date: timestamp with time zone (nullable)
...
// Table: salespersons
//   id: uuid (not null, default: gen_random_uuid())
//   salesperson_id: text (not null, default: ('SP-'::text || (nextval('salespersons_id_seq'::regclass))::text))
//   name: text (not null)
//   email: text (nullable)
//   phone: text (nullable)
//   department: text (nullable)
...
//   FOREIGN KEY customers_salesperson_id_fkey: FOREIGN KEY (salesperson_id) REFERENCES salespersons(id) ON DELETE SET NULL
// Table: departments
...
// Table: hr_settings
//   UNIQUE hr_settings_department_key: UNIQUE (department)
//   PRIMARY KEY hr_settings_pkey: PRIMARY KEY (id)
// Table: late_card_pulls
//   FOREIGN KEY late_card_pulls_created_by_fkey: FOREIGN KEY (created_by) REFERENCES auth.users(id) ON DELETE SET NULL
//   PRIMARY KEY late_card_pulls_pkey: PRIMARY KEY (id)
// Table: metrics
...
//   FOREIGN KEY purchasing_expedites_task_id_fkey: FOREIGN KEY (task_id) REFERENCES purchasing_tasks(id) ON DELETE CASCADE
//   FOREIGN KEY purchasing_expedites_wo_id_fkey: FOREIGN KEY (wo_id) REFERENCES work_orders(id) ON DELETE CASCADE
// Table: purchasing_task_audit_log
//   FOREIGN KEY purchasing_task_audit_log_changed_by_fkey: FOREIGN KEY (changed_by) REFERENCES users(id) ON DELETE SET NULL
//   PRIMARY KEY purchasing_task_audit_log_pkey: PRIMARY KEY (id)
//   FOREIGN KEY purchasing_task_audit_log_task_id_fkey: FOREIGN KEY (task_id) REFERENCES purchasing_tasks(id) ON DELETE CASCADE
// Table: purchasing_task_comments_history
//   FOREIGN KEY purchasing_task_comments_history_author_fkey: FOREIGN KEY (author) REFERENCES users(id) ON DELETE SET NULL
//   PRIMARY KEY purchasing_task_comments_history_pkey: PRIMARY KEY (id)
//   FOREIGN KEY purchasing_task_comments_history_task_id_fkey: FOREIGN KEY (task_id) REFERENCES purchasing_tasks(id) ON DELETE CASCADE
// Table: purchasing_tasks
//   FOREIGN KEY purchasing_tasks_assigned_to_fkey: FOREIGN KEY (assigned_to) REFERENCES users(id) ON DELETE SET NULL
//   PRIMARY KEY purchasing_tasks_pkey: PRIMARY KEY (id)
//   FOREIGN KEY purchasing_tasks_wo_id_fkey: FOREIGN KEY (wo_id) REFERENCES work_orders(id) ON DELETE CASCADE
// Table: quality_late_card_pulls
...
// Table: quote_versions
//   FOREIGN KEY quote_versions_created_by_fkey: FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL
//   PRIMARY KEY quote_versions_pkey: PRIMARY KEY (id)
//   FOREIGN KEY quote_versions_quote_id_fkey: FOREIGN KEY (quote_id) REFERENCES quotes(id) ON DELETE CASCADE
// Table: quotes
...
// Table: warranty_claims
//   FOREIGN KEY warranty_claims_created_by_fkey: FOREIGN KEY (created_by) REFERENCES auth.users(id) ON DELETE SET NULL
...
//     USING: true
// Table: hr_settings
//   Policy "Auth insert hr_settings" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "Auth read hr_settings" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "Auth update hr_settings" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
// Table: late_card_pulls
//   Policy "late_card_pulls_delete" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "late_card_pulls_insert" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "late_card_pulls_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "late_card_pulls_update" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: metrics
...
//     USING: true
// Table: purchasing_task_audit_log
//   Policy "Auth delete purchasing_task_audit_log" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "Auth insert purchasing_task_audit_log" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "Auth read purchasing_task_audit_log" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "Auth update purchasing_task_audit_log" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: purchasing_task_comments_history
//   Policy "Auth delete purchasing_task_comments_history" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "Auth insert purchasing_task_comments_history" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "Auth read purchasing_task_comments_history" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "Auth update purchasing_task_comments_history" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: purchasing_tasks
//   Policy "Auth delete purchasing_tasks" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "Auth insert purchasing_tasks" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "Auth read purchasing_tasks" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "Auth update purchasing_tasks" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: quality_late_card_pulls
...
//     USING: true
// Table: quote_versions
//   Policy "Auth insert quote_versions" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "Auth read quote_versions" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: quotes
...
//     USING: true
//   Policy "warranty_claims_delete" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "warranty_claims_insert" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "warranty_claims_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "warranty_claims_update" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: wo_history
...
//     USING: ((get_user_department() ~~ (('%'::text || department) || '%'::text)) OR (get_user_role() = 'admin'::text))
...
// Table: purchasing_tasks
//   on_purchasing_task_update: CREATE TRIGGER on_purchasing_task_update AFTER UPDATE ON public.purchasing_tasks FOR EACH ROW EXECUTE FUNCTION log_purchasing_task_changes()
//   on_purchasing_task_updated_at: CREATE TRIGGER on_purchasing_task_updated_at BEFORE UPDATE ON public.purchasing_tasks FOR EACH ROW EXECUTE FUNCTION update_purchasing_task_updated_at()
// Table: quotes
//   on_quote_insert_version: CREATE TRIGGER on_quote_insert_version AFTER INSERT ON public.quotes FOR EACH ROW EXECUTE FUNCTION save_quote_version()
...
//   on_quote_update_version: CREATE TRIGGER on_quote_update_version AFTER UPDATE ON public.quotes FOR EACH ROW WHEN ((old.* IS DISTINCT FROM new.*)) EXECUTE FUNCTION save_quote_version()
// Table: salespersons
...
// Table: hr_settings
//   CREATE UNIQUE INDEX hr_settings_department_key ON public.hr_settings USING btree (department)
// Table: notifications
