...
export default function ComponentsTab({ woFilter }: { woFilter: string }) {
  ...
  const fetchData = async () => {
    setLoading(true)
    let query = supabase.from('purchasing_components').select('*, work_orders!inner(wo_number)')
          ...
          {loading ? (
            ...
            data.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{item.expected_delivery_date || '-'}</TableCell>
