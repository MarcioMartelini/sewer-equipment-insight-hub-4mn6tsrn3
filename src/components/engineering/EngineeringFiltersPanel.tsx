...
import { EngineeringFilters } from '@/services/engineering-dashboard'

interface Props {
  filters: EngineeringFilters
  updateFilter: (key: keyof EngineeringFilters, value: any) => void
  resetFilters: () => void
}

export function EngineeringFiltersPanel({ filters, updateFilter, resetFilters }: Props) {
    ...
    >
      ...
      <CollapsibleContent>
        <div className="p-4 pt-0 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 border-t border-slate-100 mt-2">
          ...
          <div className="space-y-2">
            <Label className="text-xs text-slate-500">Designer</Label>
            <Input
              className="h-8 text-sm"
